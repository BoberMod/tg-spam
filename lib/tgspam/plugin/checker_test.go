...
func TestChecker_MetaFields(t *testing.T) {
	...
	err := os.WriteFile(scriptPath, []byte(`
		function check(req)
			if req.meta.has_contact and req.meta.message_id == 42 and req.meta.chat_id == 100 and
				req.meta.has_giveaway and req.meta.has_external_reply and req.meta.has_document then
	...
	t.Run("all meta fields set", func(t *testing.T) {
		resp := checkFunc(spamcheck.Request{
			...
			Meta: spamcheck.MetaData{
				ChatID: 100, HasContact: true, MessageID: 42, HasGiveaway: true, HasExternalReply: true, HasDocument: true,
			},
		})
		assert.True(t, resp.Spam)
